...
export default function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-6">
      ...
      {program && <p className="mb-4 text-sm text-gray-500">{program.name}</p>}
      ...
      {loading ? (
        ...
        <div className="mb-8 text-center">
          <p className="mb-4 text-gray-500">
            No active program. Create one to get started!
          </p>
          <Link
          ...
          {recentSessions.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentSessions.slice(0, 5).map((session) => {
                const routine = program.routines.find(
                  (r) => r.id === session.routine_id,
                );
                const accent = routine?.accent || "#7F77DD";
                const short =
                  routine?.short || session.day_name?.charAt(0) || "?";
                return (
                  <Link
                    key={session.id}
