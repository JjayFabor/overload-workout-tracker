...
export default function DashboardPage() {
  ...
  const { data: sessions = [], isLoading: sessionsLoading } = useSWR<WorkoutSession[]>(
    '/api/sessions',
    fetcher,
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-6">
      ...
      {program && (
        <p className="mb-4 text-sm text-gray-500">{program.name}</p>
      )}
          ...
          <div className="mb-8 grid grid-cols-2 gap-3">
            {program.routines.map((routine) => {
              ...
              return (
                ...
                >
                  <div className="mb-2 text-sm font-medium" style={{ color: routine.accent }}>
                    {routine.label}
                  </div>
                  <div className="text-lg font-semibold text-gray-900">{routine.name}</div>
            ...
            <div className="space-y-3">
              {recentSessions.slice(0, 5).map((session) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <div className="font-medium text-gray-900">{session.day_name}</div>
                      <div className="text-sm text-gray-500">{formatDate(session.completed_at)}</div>
                    </div>
                    <div
